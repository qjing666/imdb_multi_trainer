...
void ThreadedRunTrain(
    const std::unique_ptr<paddle::framework::ProgramDesc>& main_program,
    ...
    for (auto name : input_feed_names) {
        readers[thread_id]->AddFeedVar(scope.Var(name), name);
    }
    ...
    LOG(INFO) <<"thread: "<<thread_id <<"; epoch: " << epoch << "; average loss: " << average_loss;
 } 
...
int main(int argc, char* argv[]) {
  ...
  for (int epoch = 0; epoch < epoch_num; ++epoch) {
    ...
    dataset_ptr->SetThreadNum(num_thread);
    ...
    for (int i = 0; i < num_thread; ++i) {
        threads.push_back(new std::thread(
            ThreadedRunTrain, std::ref(main_program), &executor, &scope,&readers,i));
    }
